...
class _AuthorPageState extends State<AuthorPage> {
  ...
  void initState() {
    ...
    context.read<SocialBloc>().add(
      CheckFollowingStatusEvent(widget.currentUser.id, widget.authorId),
    );
  ...
  Widget build(BuildContext context) {
    ...
    final data = _authorData!;

    return Scaffold(
      appBar: AppBar(title: const Text('Tác giả')),
      body: SingleChildScrollView(
        child: Column(
          children: [
            ...
            Text('Followers: ${(data['followers']?.length ?? 0)}'),
            const SizedBox(height: 8),
